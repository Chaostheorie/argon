    ...
    for (let i = 0; i < tmp.mails.length; i++) {
      if (tmp.mails[i].is_unread === 1) {
        $('#main-table').DataTable().row.add([
          "", tmp.mails[i].subject, {display: moment(tmp.mails[i].date * 1000).format("DD.MM.YYYY, hh:mm"), val: tmp.mails[i].date}, tmp.mails[i].from[0].name, {display: "<i class='fas fa-eye black-text'></i>", val: 1}, tmp.mails[i].id
        ]);
      } else {
        $('#main-table').DataTable().row.add([
          "", tmp.mails[i].subject, {display: moment(tmp.mails[i].date * 1000).format("DD.MM.YYYY, hh:mm"), val: tmp.mails[i].date}, tmp.mails[i].from[0].name, {display: "<i class='fas fa-eye text-info'></i>", val: 0}, tmp.mails[i].id
        ]);
      }
